...
function CoachForm({ coachData, setCoachData, schools, isEdit = false }) {
    return (
        <div className="grid gap-4 py-4">
            ...
            <div>
                <Label htmlFor="coach_phone">Phone (optional)</Label>
                <Input id="coach_phone" value={coachData.coach_phone || ""} onChange={(e) => setCoachData({...coachData, coach_phone: e.target.value})} />
            </div>
            <div>
...
function BulkImportModal({ isOpen, onClose, athlete, schools, onSave }) {
    ...
    const handleParse = async () => {
        ...
        const prompt = "Extract football coach contact info (name, title, email, twitter, phone) from this content for " + (school?.name || "the school") + ":\n\n" + websiteContent;
        try {
            const result = await InvokeLLM({
                prompt,
                response_json_schema: { type: "object", properties: { coaches: { type: "array", items: { type: "object", properties: { name: { type: "string" }, title: { type: "string" }, email: { type: "string" }, twitter: { type: "string" }, phone: { type: "string" } }, required: ["name", "title"] } } } }
            });
        ...
        <div className="flex justify-between items-start mb-8">
            <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                    <h1 className="text-3xl font-bold text-slate-900">Coach Contact Management</h1>
                    <p className="text-slate-600">Add, edit, and manage all coach contacts for your target schools.</p>
                </div>
            </div>
        </div>
                                        ...
                                        {school.coaches.length > 0 ? (
                                            <div className="space-y-3">
                                                {school.coaches.map(coach => (
                                                    <Card key={coach.id} className="p-4 bg-slate-50 border shadow-sm">
                                                        <div className="flex justify-between items-start">
                                                            <div>
                                                                <h4 className="font-semibold text-base">{coach.coach_name}</h4>
                                                                <p className="text-sm text-slate-700">{coach.coach_title}</p>
                                                                <div className="mt-2 space-y-1 text-xs text-slate-600">
                                                                    {coach.coach_email && <p>📧 <a href={`mailto:${coach.coach_email}`} className="hover:underline">{coach.coach_email}</a></p>}
                                                                    {coach.coach_phone && <p className="text-xs text-slate-500">📞 {coach.coach_phone}</p>}
                                                                    {coach.coach_twitter && <p>🐦 <a href={`https://twitter.com/${coach.coach_twitter.replace('@','')}`} target="_blank" rel="noopener noreferrer" className="hover:underline">{coach.coach_twitter}</a></p>}
                                                                </div>
                                                            </div>
